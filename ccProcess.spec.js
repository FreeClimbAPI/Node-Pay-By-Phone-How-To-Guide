...
describe('POST /ccProcess',()=>{
    ...
    it('returns redirect to /ccNumberPrompt if the on an unsuccessful response from the payment processor (fake)',async ()=>{
        ...
        expect(res.body).toStrictEqual([
            {
                Say:{
                    text:'We\'re having trouble processing your payment, please try again. Remember we Accept Visa, Discover, Mastercard and American Express cards'
                }
            },
            {
                Redirect: {
    ...
    it('returns redirect to /transfer if max unsuccessful response limit reached',async ()=>{
        ...
        expect(res.body).toStrictEqual([
            {
                Say:{
                    text:'The payment could not be processed at this time please wait while we transfer you to an operator'
                }
            },
            {
                Redirect: {
